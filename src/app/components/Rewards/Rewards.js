...
const Rewards = () => {
  ...
  // Effects
  useEffect(() => {
    if (session?.user?.country_id !== 188 && session?.user?.country_id !== 227) {
        ...
        <button
          className={`${styles.redeemButton} ${
            !(minimum_points_to_redeem < userPoints && 
              userBalance >= reward.value_in_votly) || submitting
              ? styles.disabled
              : ''
          }`}
            ...
            <div className={styles.balanceContent}>
              ...
              <div className={styles.balanceDetails}>
                ...
                <p className={styles.balanceAmount}>
                  ...
                  <span className={styles.amount}>{userBalance.toFixed(2)}</span>
                </p>
              </div>
            </div>
                  ...
                  <tbody>
                    {redeemHistory.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td>{item.value_in_votly} {usedCurrency}</td>
                        <td>{locale === 'en' ? item.brand_en : item.brand_ar}</td>
                        <td>
